...
The following tables record the main full-run ledgers inside the manuscript so the paper can be read without opening external experiment files. The complete machine-readable ledgers remain in the project repository and should be treated as the reproducibility source of record. Table~\ref{tab:model-mode-full} gives the complete model-by-mode metric profile, and Table~\ref{tab:failure-family-full} gives the corresponding family-level failure counts.
