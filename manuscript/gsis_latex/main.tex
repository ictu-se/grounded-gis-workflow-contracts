...
\subsection{Full-run diagnostic ledgers}

The aggregate results above establish the main empirical pattern: grounding removes many data hallucinations, GeoGuard adds a narrower CRS and execution-readiness gain, and residual failures concentrate in field and map-contract decisions. The final results subsection opens the underlying ledgers that support those claims, so the reader can see whether the summary statistics hide model- or family-specific exceptions. Table~\ref{tab:model-mode-full} gives the complete model-by-mode metric profile, while Table~\ref{tab:failure-family-full} gives the corresponding family-level failure counts. The complete machine-readable ledgers remain in the project repository and should be treated as the reproducibility source of record.
...
\subsection{Compact task and hard-case ledgers}

The model and family ledgers still average over individual task contracts. The compact ledgers below close the Results section by showing the task suite, the per-task mode response, the strongest-prompt model-family surface, and the remaining hard cases. Table~\ref{tab:task-suite-summary} summarizes the 30-task suite without listing task-level filenames, Table~\ref{tab:task-prs-matrix} gives a task-level PRS matrix across modes, Table~\ref{tab:geoguard-model-family-matrix} gives a GeoGuard model-by-family matrix, and Table~\ref{tab:geoguard-hard-cases} lists the lowest-scoring GeoGuard task-model cases after removing malformed JSON responses. Together, these tables make the experimental basis explicit without printing every generated row.
...
Code, prompts, model-panel definitions, generated workflow specifications, task-level scores, aggregate tables, and reproduction scripts are available at \url{https://github.com/ictu-se/grounded-gis-workflow-contracts}. The repository provides the reproducible workflow-contract benchmark artifact for this study; raw third-party geospatial datasets are not redistributed.
